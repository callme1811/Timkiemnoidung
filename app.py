...
# RETRIEVAL
# =========================
def select_relevant_nodes(question,
                          nodes,
                          model,
                          top_k=3):
...
with st.sidebar:
    ...
    md_path = st.text_input(
        ...
        value="/workspaces/Timkiemnoidung/technova_ai_demo_data.md"
    )
...
if build_button:
    ...
    if not Path(md_path).exists():
        ...
        with st.spinner("Đang build tree..."):
            ...
            code, stdout, stderr = build_pageindex_tree(
                md_path,
                model
            )
