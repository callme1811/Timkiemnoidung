...
# Base folder luôn là thư mục chứa app.py
...
def normalize_path(path_text: str) -> Path:
    """Chuẩn hóa path tương đối/absolute"""
...
def build_markdown_tree(md_path: Path):
    ...
    if not flat_nodes:
        flat_nodes = [{"title": md_path.stem, "level": 1, "line_num": 1, "nodes": []}]
...
def select_relevant_nodes(question, nodes, model, top_k=3, use_ollama=True):
    if not use_ollama:
        ranked = sorted(nodes, key=lambda n: simple_keyword_score(question, n.get("text", "")), reverse=True)
        return ranked[:top_k], "keyword fallback"

    node_list = "\n".join([f"- {n['node_id']}: {n['path']} (lines {n['start_line']}-{n['end_line']})" for n in nodes])
    prompt = f"""
...
with st.sidebar:
    st.header("Cấu hình")
    md_path_input = st.text_input("Đường dẫn file Markdown", value=str(DEFAULT_MD_PATH))
    model = st.text_input("Ollama model", value=DEFAULT_MODEL)
    use_ollama = st.checkbox("Dùng Ollama để trả lời", value=True)
    st.markdown("---")
    build_button = st.button("1. Build Tree")
    st.info("Codespaces dùng Ollama local được. Streamlit Cloud thường không gọi được 127.0.0.1:11434.")
...
if tree_path.exists():
    ...
    question = st.text_input("Nhập câu hỏi", value="Quy trình xử lý sự cố production gồm những bước nào?")
    top_k = st.slider("Số node retrieval", min_value=1, max_value=5, value=3)

    if st.button("Tìm kiếm và trả lời"):
        with st.spinner("Đang tìm kiếm..."):
            selected_nodes, raw_selection = select_relevant_nodes(question, nodes, model, top_k, use_ollama)
            answer, context_text = answer_question(question, selected_nodes, model, use_ollama)
    ...
    st.warning("Chưa thấy file tree JSON. Hãy bấm Build Tree trước.")